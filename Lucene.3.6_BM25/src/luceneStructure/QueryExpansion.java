...
public class QueryExpansion {
	...
	//Builder
	//--------------------------------------------------
	public QueryExpansion( Analyzer analyzer, org.apache.lucene.search.Searcher searcher, Similarity similarity, Properties prop ) throws CorruptIndexException, IOException {
		this.analyzer = analyzer;
		this.searcher = searcher;
		this.similarity = similarity;
		this.prop = prop;
		this.queryParser = new QueryParser(Version.LUCENE_36, LuceneConstants.CONTENTS, analyzer);
		this.indexSearcher = new IndexSearcher(FSDirectory.open(new File(LuceneConstants.INDEX_DIR)));
	}

	//--------------------------------------------------
	//Method fixing the number of documents whose terms will be analyzed for the extension and returning the extended request
	//--------------------------------------------------
	public Query obtainExpandedQuery( String queryStr, Vector<Document> hits, Properties prop )
					...
					throws IOException, ParseException
	{
		...
		expandedQuery = mergeQueries( expandedQueryTerms, queryTerms, maxExpandedQueryTerms, queryTermsNumber, alpha, beta );
	...
	//--------------------------------------------------
	public Query mergeQueries( Vector<TermQuery> termQueries, Vector<TermQuery> queryTerms, int maxTerms, int nbInitQueryTerms, float alpha, float beta)
			throws ParseException
	{
		...
		for ( int i = 0; i < termCount; i++ )
		{
			...
			TermQuery originalQueryTerm = find(termQuery, queryTerms);
			if (originalQueryTerm != null) {
				termQuery.setBoost(alpha);
			} else {
				termQuery.setBoost(beta);
			}
			/**
			 * Debugging code
			 */
			...
			throws IOException
	{
		...
		int nbDocsTerms = LuceneConstants.TOTAL_DOCS_TERMS;
		int nbDocs = indexReader.numDocs();
		float avgDocsLength = nbDocsTerms/nbDocs;
		...
		for ( int g = 0; g < docsTerms.size(); g++ )
		{
			...
			int docLength = docTerms.getTerms().length;
			...
			for ( int i = 0; i < docTerms.size(); i++ )
			{
				...
				//Create BM25 conbined weight for 
				float tf = ((LuceneConstants.BM25_K1 +1) * termFrequencies[i]) / (LuceneConstants.BM25_K1 * (1 - LuceneConstants.BM25_B ) + (LuceneConstants.BM25_B * docLength/avgDocsLength) + termFrequencies[i]);
				...
				//System.out.println("doc length : " + docLength + " docs average length : " + avgDocsLength);
	...
	//--------------------------------------------------
	// Method that calculates the average length of the documents
	//--------------------------------------------------
	public float averageDocsLength() throws CorruptIndexException, IOException {
		IndexReader indexReader = indexSearcher.getIndexReader();
		int docsCumulatedTerms = 0;
		for (int i = 0; i < indexReader.maxDoc(); i++) {
			if (indexReader.isDeleted(i))
				continue;
			Document doc = indexReader.document(i);
			StringBuffer docTxtBuffer = new StringBuffer();
			String[] docTxtFlds = doc.getValues( LuceneConstants.FILE_TEXT );
			for ( int j = 0; j < docTxtFlds.length; j++ )
			{
				docTxtBuffer.append( docTxtFlds[j] + " " );
			}      
			...
			//Create termVector and add it to vector
			QueryTermVector docTerms = new QueryTermVector( docTxtBuffer.toString(), analyzer );
			docsCumulatedTerms = docsCumulatedTerms + docTerms.size();
		}
		
		return (docsCumulatedTerms/LuceneConstants.TOTAL_DOCS_TERMS);
	}
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		//Formulating a query
		String searchQuery = "coronavirus diabetes what kinds of complications related to COVID-19 are associated with diabetes"; //User Query
		...
		String [] relevantsDocs = {"4cx6fe5v.txt", "jg6v644y.txt", "ol2exczc.txt"};
		...
		Query queryExtend = queryExp.obtainExpandedQuery(searchQuery, hits , prop);
		System.out.println("Extended query : " + queryExtend.toString());

		//Print average docs length
		//System.out.println(queryExp.averageDocsLength());
